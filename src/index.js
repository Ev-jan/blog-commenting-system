...
if (commentSectionNode && commentThreadNode) {
    ...
    if (commentForm) {
        ...
        const retrievedCommentThread = commentThread.getCommentThread();
    ...
    //   }
    // }
    async function renderComments(retrievedCommentThread) {
        for (const commentString of retrievedCommentThread) {
            const threadItem = JSON.parse(commentString);
            if (threadItem.hasOwnProperty("ownId") && threadItem.hasOwnProperty("parentId") === false) {
                const comment = new CommentComponent(currentUserName, currentUserAvatar, threadItem.timeStamp, threadItem.text, threadItem.ownId, threadItem.votes, threadItem.isAddedTofavourite, threadItem.storedUser, threadItem.storedAvatar);
                comment.createComment(commentThreadNode);
                comment.updateComment();
            }
            else if (threadItem.hasOwnProperty("parentId")) {
                const reply = new ReplyComponent(threadItem.currentUser, threadItem.currentUserAvatar, threadItem.timeStamp, threadItem.text, threadItem.ownId, threadItem.votes, threadItem.isAddedTofavourite, threadItem.storedUser, threadItem.storedAvatar);
